...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, role }) => {
    ...
    if (!user || !user.token) {
        // Redirecionar para login, mas salvar a localização que eles tentaram acessar
        return <Navigate to="/login" state={{ from: location }} replace />;
    }

    if (role) {
        const userRole = user.role ? user.role.toUpperCase() : '';
        const userRoles = user.roles ? user.roles.map((r: string) => r.toUpperCase()) : [];
        const requiredRole = role.toUpperCase();

        // Verifica string simples
        const matchString = userRole === requiredRole ||
            userRole === `ROLE_${requiredRole}` ||
            (requiredRole === 'ADMIN' && userRole === 'ROLE_ADMIN');

        // Verifica array
        const matchArray = userRoles.includes(requiredRole) ||
            userRoles.includes(`ROLE_${requiredRole}`);

        if (!matchString && !matchArray) {
            // Se o usuário não tem a role necessária, mostrar UI de Acesso Negado
            return (
                        ...
                        <div className="flex flex-col items-center text-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-cyber-error/80 font-mono text-sm leading-relaxed mb-4">
                                    ...
                                    <br />
                                    Credenciais insuficientes para acesso.
                                </p>
                            </div>
